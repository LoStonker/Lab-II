...
def retta_calibrazione(x, m, q):
    """ Modello lineare per la calibrazione energetica (E = m * CH + q) """
    return m * x + q
...
class FitLikelihoodBomberone:
    ...
    def plot_results(self, show_hist=True, info_box_pos='upper right', componenti=None):
        ...
        Args:
            show_hist: se True, mostra anche l'istogramma e le barre d'errore.
            ...
            componenti: lista opzionale di tuple (label, funzione) per plottare
                        ...
                        Esempio: [('Gaussiana', gauss_pdf), ('Parabola', parab_pdf)]
        """
        ...
        if show_hist:
            # Aggiunta l'opzione per disegnare l'istogramma a gradini
            ax1.plot(self.x, self.y, drawstyle='steps-mid', color='royalblue', alpha=0.5, label='Istogramma')
        ...
        ax1.plot(x_fit, y_fit, color='red', linewidth=10, label='Fit Extended Likelihood')

        # --- Plotta le singole componenti se fornite ---
        if componenti is not None:
            colori_comp = ['green', 'orange', 'purple', 'brown', 'pink', 'gray']
            for i, (label_comp, func_comp) in enumerate(componenti):
                colore = colori_comp[i % len(colori_comp)]
                y_comp = func_comp(x_fit, **kwargs) * bin_width
                ax1.plot(x_fit, y_comp, color=colore, linewidth=1.5,
                         linestyle='--', label=label_comp)
        ...
        ax1.annotate(res_text, xy=box_coords['xy'], xycoords='axes fraction',
                     va=box_coords['va'], ha=box_coords['ha'], fontsize=10,
